...
public class DefaultPerVertexLightingApplyFunction implements PerVertexLightingApplyFunction {
    ...
    public void appendFunction(Renderable renderable, FragmentShaderBuilder fragmentShaderBuilder, boolean hasSpecular) {
        if (hasSpecular) {
            fragmentShaderBuilder.addFunction("defaultApplyLighting",
                    "vec3 defaultApplyLighting(vec3 color, vec3 diffuse, vec3 specular) {\n" +
                            "  return color * diffuse + specular;\n" +
                            "}\n");
        } else {
            fragmentShaderBuilder.addFunction("defaultApplyLighting",
                    "vec3 defaultApplyLighting(vec3 color, vec3 diffuse) {\n" +
